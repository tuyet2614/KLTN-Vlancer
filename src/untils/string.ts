 ...
 */
export const formatNumberStr = (number: number | null | undefined, decimalLength = 3) => {
  ...
  if (number.toString().includes('.')) {
    ...
    return `${evenSection.replace(/\B(?=(\d{3})+(?!\d))/g, '.')},${decimalSection.substring(0, decimalLength)}`;
  }
  return `${number}`.replace(/\B(?=(\d{3})+(?!\d))/g, '.');
};
export const formatNumberStrRounded = (number: number | null | undefined, decimalLength = 3) => {
  if (number === 0) return '0';
  if (!number || isNaN(number)) return '';
  if (number.toString().includes('.')) {
...
export const numberParser = (value: string | undefined, decimalLength = 3) => {
  if (value?.toString().includes(',')) {
    ...
    const decimalSection = numberParseArray[1];
    decimalLength = decimalLength > decimalSection.length ? decimalSection.length : decimalLength;
    return Number(`${evenSection.replaceAll('.', '')}.${decimalSection.substring(0, decimalLength)}`);
  }
